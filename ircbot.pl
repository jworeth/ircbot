...
#default Options
my $network = "irc.rizon.net"; #Network to connect to
my @channels = ("#demotesting"); #Channels to join in
my $op = "this.is.mynet";
my $nick = "Demouser"; #Nick bot uses
my $user = "DU \"localhost\" \"".$network."\" :DU"; #Username
my $chancount = 0;
...
my $socket = IO::Socket::INET->new(PeerAddr => $network, PeerPort => 6667, Proto => "tcp", Type => SOCK_STREAM) or die("Can't Connect");
send_raw ("user $user\n");
send_raw ("nick $nick\n");
while(<$socket>){
   parse_cmds($_);
